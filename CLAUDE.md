...
- 4 solver runs, one per constraint regime (UNIVERSAL_ONLY, B_ANCHORED, M_ANCHORED, BOTH_ANCHORED). **Convexity finding (June 2026):** predicted price is linear in (B, M) with V fixed pre-solve, so each regime is a convex QP with a unique optimum — multiple initialization strategies cannot disagree, and the historical "local minima" were an Excel GRG artifact. Production uses the OUTSIDE_IN start (feasible for every regime); dual-init agreement is enforced by the golden test suite, not at runtime
- **Winner selection rule:** lowest weighted SSE among valid converged runs; numerical ties (within 1e-6 relative) resolve to the least-constrained regime. The regimes nest — SSE(UNIVERSAL_ONLY) ≤ all anchored regimes always — so the unconstrained solution wins in practice; anchored solutions exist as user-override alternatives only. Universal constraints: B ≥ 0, M > B + ε
...
- **Phase 4:** Per-respondent and aggregated CR computation, include/exclude toggle; "Review →" link in per-respondent expanded CR view (scoped to FACILITATED + current user email); collapsible External Respondents management panel (add/copy-link/unlock/remove distributed respondents); dedup check on add (email+project+mode query before insert — shows error if already present); submitted respondents show completion time inline (derived from `survey_started_at` and `submitted_at`); post-solve diagnostics banner; auto-heal: if status=SURVEY_OPEN but aggregated_matrix exists, restores to SURVEY_CLOSED and calls router.refresh(); AI coherence summary — "✦ AI Summary" button inline with description (right-aligned), card renders above flag banners, calls `explain_coherence` task in `/api/ai/route.ts`, 3–4 sentence plain-language interpretation of survey consistency; HelpTip tooltips on Respondents heading (include/exclude behavior) and Aggregated Coherence Scores heading (threshold guide + remediation); **Respondent Consensus section** (≥2 included respondents) — per-comparison-set Consensus Score badge via `/consensus`, expandable divergent-pair drill-down naming who anchored each side; computed during Run Analysis, not persisted/rehydrated
- **Phase 5:** Loads weights/utilities/assignments, calls /solve, model fit display (RMSE + NRMSE + RSE), 4-run solver table (one row per constraint regime; Init column removed after 8→4 collapse) with B/M/R²/NRMSE/RSE/target estimates, reference product positioning, price recommendations (statistical + market envelope ranges), auto-rehydration on return visit, solver run override (select any of 8 runs as active, saves to DB), post-solve diagnostics (value scale coverage, market share concentration, R² reliability, factor weight concentration — all at 2.5× equal-share threshold); AI diagnostic explanations — background call after solver run, plain-language interpretation displayed in "Model Interpretation" section; **autoRunSolver writes results back to DB** (updates `regression_result` B/M/SSE/R² and `target_score` normalized_score/point_estimate/ranges) — prevents Phase 6 showing $0 after Phase 2 re-save wipes target_score solver fields; solver errors surface plain-language messages (503 → service unavailable, convergence failure → descriptive fallback); HelpTip tooltips on Model Diagnostics, Reference Product Positioning, Benchmark Outlier Review, and Price Recommendations headings; **run selection recomputes benchmark residuals and outlier flags** from new B/M so reference product positioning stays in sync; **Predictive Error tile** (LOO RMSE via `run_loo: true`, bands 10%/20% of avg price) + influence flag banner when LOO stability > 25% (names the benchmark); **Market Influence selector** — segmented control (Market-weighted/Balanced/Equal) next to Run button, persists to `project.weight_mode`, change invalidates results and requires re-run; all Phase 5/6/7 reads use resilient separate `.select('weight_mode')` queries (missing column can't break page load); **Market Influence Check** strip — background re-solves under the two non-active modes after each solve, active-mode row first (bold, "(active)" suffix), spread column and ±2.5% robustness verdict; Phase 6 shows amber "Market Influence: Balanced/Equal" badge on Price Recommendations when non-default; Phase 7 solves all pass the project's weight_mode
- **Phase 6:** Value Map with 5-dot target strip, Factor Contributions lollipop + stacked bar, Competitive Positioning Table, Price Recommendations with gap analysis; HelpTip tooltips on Value Map, Factor Contributions, and Price Recommendations headings; footer links to Phase 7; AI Positioning Narrative — "Positioning Narrative" section above Value Map, "✦ Generate Narrative" button, copy-to-clipboard, regenerate action; calls `generate_narrative` task in `/api/ai/route.ts`, 4–6 sentence executive-level summary (no methodology jargon); `target_segment` passed to narrative payload and included in AI prompt; PDF export button removed (moved to Phase 7)
...
- `run_solver()` — 4 runs (one per constraint regime), returns all_runs array with r_squared, rse, and target_point_estimates per run; winner = lowest SSE with least-constrained tie-break; accepts `weight_mode`
- `compute_observation_weights()` — isolated weight computation: `market_share` (default), `sqrt_share` ("Balanced"), `equal`; flows through /solve, LOO, and sensitivity
- `run_loo_cv()` — leave-one-out CV ("Predictive Error"): per-benchmark exclusion + full refit + held-out prediction; share-weighted LOO RMSE mirrors in-sample RMSE semantics; stability = max relative B/M movement (denominators guarded at 1% of mean price); skips below 4 benchmarks. **Item 3 Step 0 finding: exclude-and-rerun did NOT pre-exist** (Phase 7 tornado perturbs prices, factor sensitivity excludes factors, Outlier Review exclude is manual) — run_loo_cv is the shared utility; future Outlier Review should rank by |LOO residual|
- `consensus_analysis()` — between-respondent agreement per comparison set: per-pair geometric SD of ratios, agreement = 1 − min(s/ln9, 1), set score = mean agreement % (STRONG ≥80, MODERATE ≥60, LOW), pairs flagged at gsd ≥ 2.5 with anchor respondents named; `/consensus` endpoint
- `price_recommendation()` — point_estimate ± std_dev of residuals
- `run_sensitivity_analysis()` — excludes one factor at a time, re-normalizes weights, uses compute_scaled_score; accepts `weight_mode`
- `SOLVER_VERSION` constant — bump ONLY on deliberate behavior changes; golden tests fail until goldens are regenerated (this is intentional)

## Solver golden tests (`vpm-app/solver/tests/`)
- **Run `python -m pytest tests -q` from `solver/` (venv active) after ANY solver change. Golden tests are the source of truth for solver behavior.**
- 4 frozen fixtures (minimal / standard / concentrated / degenerate) + golden files locking weights, CRs, value indices, WLS solutions, sensitivity, recommendations
- Named guards: `test_option2_not_option1` (formula revert), `test_wls_not_ols` (weights dropped), `test_dual_init_agreement` (convexity regression), `test_regime_nesting`, `test_anchors_are_utility_driven_not_display_order`
- **Never regenerate goldens to make failing tests pass** — regenerate only on deliberate behavior change, with SOLVER_VERSION bump + commit-message justification (policy in `tests/README.md`)
- `fixture_standard` is synthetic (Supabase was paused at suite creation); can be replaced with a CO2-project export later, regenerating goldens
...
- `response_ms` column on `pairwise_response`: per-question response time (display → save), captured in both survey paths (Phase 3 in-app and `/survey/[token]`), capped at 1h, written via resilient separate update after the main insert — a missing column never breaks the survey flow. No UI yet (Proposal I instrumentation; straight-lining/low-effort detection later)
- `survey_started_at` migration verified applied (June 2026)
...
- ~~is_ordinal + survey_started_at migrations~~ — verified applied (June 2026, post-unpause column probe)
- **Pending DB migrations (run in Supabase SQL editor):**
  - `ALTER TABLE project ADD COLUMN IF NOT EXISTS weight_mode text NOT NULL DEFAULT 'market_share';` — persistent Market Influence setting (Item 4)
  - `ALTER TABLE pairwise_response ADD COLUMN IF NOT EXISTS response_ms integer;` — per-question response timing (Proposal I instrumentation)
