...
| 🟡 | ✓ Per-question response timing (Improvements Spec Proposal I instrumentation) | DONE — `response_ms` captured on every pairwise response in both survey paths (Phase 3 in-app + distributed `/survey/[token]`), measured client-side from question display to save, capped at 1h, written via resilient separate update. UI (straight-lining detection, low-effort flags in Phase 4) deferred until real distributed data accumulates. Migration: `ALTER TABLE pairwise_response ADD COLUMN IF NOT EXISTS response_ms integer;` |
...
| 🟡 | ✓ LOO cross-validation — "Predictive Error" (Improvements Spec Item 3) | DONE — Step 0 finding: exclude-and-rerun did not pre-exist; `run_loo_cv()` built as shared utility (future Outlier Review should rank by \|LOO residual\|). Phase 5: 5th fit tile with 10%/20% bands + influence flag banner at stability > 25%. Not persisted to DB (autoRunSolver recomputes on every visit); loo_rmse columns optional later for DB-only PDF path. |
| 🟡 | ✓ Weighting mode toggle — "Market Influence" (Improvements Spec Item 4) | DONE — `compute_observation_weights()` (market_share / sqrt_share "Balanced" / equal), `weight_mode` flows through solve/LOO/sensitivity, golden-tested. Phase 5: segmented Market Influence selector next to Run button (persists to `project.weight_mode`, change invalidates results), "Market Influence Check" strip with active-mode-aware rows + ±2.5% robustness verdict. Phase 6: amber badge on Price Recommendations when non-default. Phase 7: all solves use the project setting. All reads use resilient separate queries (page loads survive a missing column). Migration: `ALTER TABLE project ADD COLUMN IF NOT EXISTS weight_mode text NOT NULL DEFAULT 'market_share';` |
| 🟡 | ✓ Respondent Consensus diagnostic (Improvements Spec Proposal A) | DONE — `consensus_analysis()` + `/consensus` endpoint: per-pair geometric SD across respondents, set-level Consensus Score (STRONG/MODERATE/LOW at 80/60), divergent pairs flagged at ×2.5 spread with anchor respondents named. Phase 4 "Respondent Consensus" section with expandable drill-down (shows with ≥2 included respondents). Computed at Run Analysis time, not persisted — re-run to see it after reload. Thresholds are initial calibrations; tune against real workshops. |
