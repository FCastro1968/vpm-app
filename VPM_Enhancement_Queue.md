...
| 🟡 | ◐ Weighting mode toggle — "Market Influence" (Improvements Spec Item 4) | Solver + robustness strip DONE — `compute_observation_weights()` (market_share / sqrt_share "Balanced" / equal), `weight_mode` flows through solve/LOO/sensitivity, golden-tested incl. mode divergence on concentrated fixture. Phase 5 "Market Influence Check" strip: background re-solves, side-by-side estimates, ±2.5% robustness verdict. **Remaining (needs Supabase unpause):** `weight_mode` column migration + persistent mode selector + non-default badge on Phase 6/exports. |
