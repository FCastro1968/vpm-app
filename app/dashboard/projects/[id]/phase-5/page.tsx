        ...
        {solverResult && (
          <>
            ...
            {(() => {
              ...
              return (
                <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <h2 className="text-base font-semibold text-gray-900 mb-3">Model Diagnostics</h2>
            ...
            <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="text-base font-semibold text-gray-900 mb-4">Reference Product Positioning</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm border-separate border-spacing-0">
                  <thead>
                    <tr>
              ...
              <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm border-separate border-spacing-0">
                      <thead>
                        <tr>
                          <th className="text-left text-xs font-medium text-gray-500 pb-3 pr-4 border-b border-gray-100" style={{ minWidth: '140px' }}>Factor</th>
                          ...
                          <th className="text-left text-xs font-medium text-gray-500 pb-3 border-b border-gray-100" style={{ minWidth: '180px' }}>Signal</th>
                        </tr>
                      </thead>
                      <tbody>
                        {solverResult.sensitivity.map((row, i) => {
                          const factor = factors.find(f => f.id === row.excluded_attribute_id)
                          const weight = attributeWeights[row.excluded_attribute_id] ?? 0
                          const fullR2 = solverResult.r_squared_weighted
                          const excR2 = row.r_squared_weighted
                          const r2Delta = excR2 != null ? excR2 - fullR2 : null
                          const priceDeltaPct = row.delta_from_full_model != null && solverResult.target_results[0]?.point_estimate
                            ? Math.abs(row.delta_from_full_model) / solverResult.target_results[0].point_estimate
                            : 0

                          // Signal logic — three mutually exclusive states + blank for middle ground
                          // Amber: market prices differently than SMEs indicated (R² improves when excluded)
                          // Blue: load-bearing (R² drops meaningfully, or large price impact with stable fit)
                          // Gray: low influence on both fit and recommendation (candidate for exclusion)
                          // Blank: middle ground, nothing notable
                          let signal = ''
                          ...
                          const priceImpactLarge = priceDeltaPct > 0.05
                          const priceImpactSmall = priceDeltaPct < 0.015
                          const r2Stable = r2DeltaAbs < 0.025

                          if (r2Improves) {
                            // Amber: removing this factor improves fit — market doesn't price it as SMEs said
                            signal = 'Worth reviewing — market may not price this factor as SMEs indicated'
                            signalColor = '#b45309'
                          } else if (r2DropsLarge || (r2Drops && priceImpactLarge) || (priceImpactLarge && r2Stable)) {
                            // Blue: load-bearing in some meaningful way
                            if (r2DropsLarge && priceImpactLarge) {
                              signal = 'Load-bearing — strongly influences both fit and recommendation'
                            } else if (r2DropsLarge) {
                              signal = 'Load-bearing — removing significantly weakens model fit'
                            } else if (priceImpactLarge && r2Stable) {
                              signal = 'Drives recommendation — high value influence with stable fit'
                            } else {
                              signal = 'Load-bearing — influences fit and recommendation'
                            }
                            signalColor = '#1d4ed8'
                          } else if (priceImpactSmall && r2Stable) {
                            // Gray: low influence on both dimensions — candidate for exclusion from run
                            signal = 'Low influence — candidate for exclusion from this model run'
                            signalColor = '#9ca3af'
                          }
                          // else: blank — middle ground, nothing notable

                          return (
                            <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                              <td className="py-2 pr-4 text-gray-800 font-medium">
                                {factor?.name ?? row.excluded_attribute_id}
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-700">
                                {(weight * 100).toFixed(1)}%
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-500">
                                {(fullR2 * 100).toFixed(1)}%
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-700">
                                {excR2 != null ? `${(excR2 * 100).toFixed(1)}%` : '—'}
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-700">
                                {r2Delta != null ? `${r2Delta > 0 ? '+' : ''}${(r2Delta * 100).toFixed(1)}pp` : '—'}
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-700">
                                {row.point_estimate != null ? formatCurrency(row.point_estimate) : '—'}
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-700">
                                {row.delta_from_full_model != null
                                  ? `${row.delta_from_full_model > 0 ? '+' : ''}${formatCurrency(row.delta_from_full_model)}`
                                  : '—'}
                              </td>
                              <td className="py-2 pr-4 text-right text-gray-700">
                                {row.delta_from_full_model != null && solverResult.target_results[0]?.point_estimate
                                  ? `${row.delta_from_full_model > 0 ? '+' : ''}${(row.delta_from_full_model / solverResult.target_results[0].point_estimate * 100).toFixed(1)}%`
                                  : '—'}
                              </td>
                              <td className="py-2 text-xs" style={{ color: signalColor }}>
                                {signal}
                              </td>
                            </tr>
                          )
            ...
            {/* ── Market-Implied Weight Analysis (Tool 3) ── */}
            {(() => {
              const includedBenches = benchmarks.filter(b => b.included_in_regression)
              const K = includedBenches.length
              const N = factors.length
              const sufficient = K > N          // need K ≥ N+1
              const marginal   = sufficient && K < N + 5   // technically ok but sparse
                ...
                <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <div className="flex items-start justify-between mb-1">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        Compares your survey-derived Importance Scores against weights that best fit observed market prices — revealing where the market agrees or disagrees with your team's assessments.
                      </p>
                    </div>
                    <button
                  ...
                  {!sufficient && (
                    <div className="mt-3 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                      ⚠ Requires at least {N + 1} reference products for {N} factors ({K} currently included). Add more reference products in Phase 1 to enable this analysis.
                    </div>
                  )}
                  {marginal && (
                    <div className="mt-3 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                      ⚠ Marginal data ({K} products, {N} factors). Results are mathematically valid but interpret with caution — more reference products improve reliability.
                    </div>
                  )}
                  {impliedError && (
                    ...
                    <div className="mt-4">
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm border-separate border-spacing-0">
                          ...
                          <tbody>
                            {(() => {
                              // Scale thresholds to equal-share weight (100/N pp)
                              // Neutral < 25%, Amber 25–65%, Red > 65% of equal-share
                              ...
                              const gapPp    = (impliedW - surveyW) * 100   // percentage points
                              ...
                              const interpretation =
                                absGap < amberAt ? '' :
                                gapPp > 0
                                  ? 'Market prices this more than survey suggests'
                      ...
                      {(() => {
                        const surveyR2  = solverResult!.r_squared_weighted ?? 0
                        // Derive SST from survey result, then compute implied R²
                        const sst       = surveyR2 < 1 ? solverResult!.weighted_sse / (1 - surveyR2) : null
                        const impliedR2 = sst && sst > 0 ? 1 - impliedResult.weighted_sse / sst : null
                        const gap       = impliedR2 != null ? impliedR2 - surveyR2 : null  // positive = implied better
                        const material  = gap != null && gap > 0.03                         // >3pp R² is meaningful
                        return (
                          <div className="mt-3 flex flex-wrap gap-6 text-xs text-gray-500">
                            <span>Survey fit (R²): {(surveyR2 * 100).toFixed(1)}%</span>
                            {impliedR2 != null && <span>Market-implied fit (R²): {(impliedR2 * 100).toFixed(1)}%</span>}
                            {material
                              ...
                              : <span className="text-green-700">✓ Survey weights produce fit comparable to market-implied (gap &lt;3pp R²)</span>
                            }
                          </div>
                        )
                      })()}
